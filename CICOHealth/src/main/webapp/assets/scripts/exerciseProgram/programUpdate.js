...
function removeWorkout(element) {
  ...
  let action = workout.dataset.action;
  if (action == "update") {
    workout.parentNode.style.display = "none";
    workout.dataset.action = "delete";
  } else {
...
function addNewWorkout() {
  ...
  newWorkout.innerHTML = `
                      ...
                      <div data-action="add" class="card-body collapse workout" data-toggle="collapse" aria-expanded="false" id="workout-${newID}" data-id = "${newID}">
...
function ExerciseListRowHtml(
  ...
  return `
      <tr id = "workout-${id}-exercise-${rowID}" data-action="add">
...
function deleteExercise(btnElement) {
  ...
  let row = document.querySelector(target);
  //Check if action is add or update
  if (row.dataset.action == "update") {
    //Preserve the row to update database
...
function addExerciseToWorkout() {
  //Check if any exercise has same id
...
function createWorkoutObject(id) {
  ...
  for (let j = 0; j < numberOfExercise; j++) {
    let action = document.querySelector(
      `#create-table-${id} tbody tr:nth-child(${j + 1})`
    ).dataset.action;
...
let workoutNameInputs = document.querySelectorAll(
